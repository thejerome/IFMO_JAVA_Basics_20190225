...
public class WarAndPeaceExercise {
    ...
    public static String warAndPeace() throws IOException {
        ...
        Arrays.stream(string.split(" ")).map(String::toLowerCase).filter(s->s.length()>3).forEach(s->{d.put(s, d.getOrDefault(s, 0) + 1);});
        List<Map.Entry<String, Integer>> list = new ArrayList<>();
        d.entrySet().stream().filter(pair -> pair.getValue() >= 10).forEach(list::add);
